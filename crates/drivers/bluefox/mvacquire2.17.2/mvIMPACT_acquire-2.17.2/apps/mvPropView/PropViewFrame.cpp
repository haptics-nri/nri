...
public:
    ...
    virtual bool OnInit( void )
    {
        ...
        SplashScreenScope splashScreenScope( wxBitmap::NewFromPNGData( splashScreen_png, sizeof( splashScreen_png ) ) );
...
//-----------------------------------------------------------------------------
{
    ...
    m_pPropGridManager = new wxPropertyGridManager( m_pSettingsPanel, widPropertyGridManager, wxDefaultPosition, wxDefaultSize,
            wxPG_BOLD_MODIFIED | wxPG_SPLITTER_AUTO_CENTER | wxPG_TOOLBAR | wxPG_DESCRIPTION | wxPG_TOOLTIPS | wxTAB_TRAVERSAL | wxPGMAN_DEFAULT_STYLE );
    ...
    wxPropertyGridPage* pPGDriver = m_pPropGridManager->AddPage( "Driver Properties", PNGToIconImage( monitor_png, sizeof( monitor_png ) ) );
    wxPropertyGridPage* pPGDevice = m_pPropGridManager->AddPage( "Device Properties", PNGToIconImage( camera_png, sizeof( camera_png ) ) );
    GlobalDataStorage::Instance()->SetListBackgroundColour( pPGDriver->GetGrid()->GetCaptionBackgroundColour() );
    ...
    wxGridSizer* pDisplayGridSizer = new wxGridSizer( displayCountY, displayCountX, 0, 0 );
    ...
    */
    ...
    wxRect rect = RestoreConfiguration( displayCount );
...
//-----------------------------------------------------------------------------
{
...

    {
        ...
        pConfig->Write( wxT( "/MainFrame/selectedPropertyGrid" ), m_pPropGridManager->GetSelectedPage() );
...
//-----------------------------------------------------------------------------
{
    ...
    if( fileOutStream.IsOk() )
    {
        ...
        for( size_t i = 0; i < logFilesArray.GetCount(); i++ )
        {
            ...
            zipOutStream.WriteAll( logFilesArray[i].mb_str(), logFilesArray[i].Length() );
        }
        zipOutStream.Sync();
        zipOutStream.Close();
        fileOutStream.Close();
    ...
    {
        wxMessageBox( wxT( "Failed create zipfile with logs on Desktop.\nYou have to create the zipfile manually(Menu->Help->Save Logfiles As Zip)" ), wxT( "Cannot Create Zipfile!" ), wxOK | wxICON_EXCLAMATION, this );
    }
}
...
//-----------------------------------------------------------------------------
{
    ...
    m_stopWatch.Start();
    const wxString timingDetails = m_pDevPropHandler->OpenDriver( deviceSerial, this, static_cast<unsigned int>( m_pDisplayAreas.size() ) );
...
//-----------------------------------------------------------------------------
wxRect PropViewFrame::RestoreConfiguration( const unsigned int displayCount )
//-----------------------------------------------------------------------------
{
    ...
    m_pPropGridManager->SelectPage( selectedPropertyGrid );
...
//-----------------------------------------------------------------------------
{
    ...
    m_pMIWizards_SequencerControl->Enable( ( s.find( wSequencerControl ) != itEND ) && !boDevLive );
...
//-----------------------------------------------------------------------------
{
    ...
    {
        wxCriticalSectionLocker locker( m_critSect );
        if( !m_sequencerSetToDisplayTable.empty() && ( m_sequencerSetToDisplayTable.size() > static_cast<std::vector<long>::size_type>( data.requestInfo_.chunkSequencerSetActive_ ) ) )
        {
            index = static_cast<int>( m_sequencerSetToDisplayTable[static_cast<vector<long>::size_type>( data.requestInfo_.chunkSequencerSetActive_ )] );
        }
        else
        {
...
//-----------------------------------------------------------------------------
{
    ...
    if( pDev )
    {
        try
        {
            ...
            WizardSequencerControl dlg( this, wxString::Format( wxT( "Sequencer Control For Device %s" ), ConvertedString( pDev->serial.readS() ).c_str() ), pDev, m_pDisplayAreas.size(), m_sequencerSetToDisplayTable );
            if( dlg.ShowModal() == wxID_OK )
            {
                m_sequencerSetToDisplayTable = dlg.GetSetToDisplayTable();
                const vector<int64_type>& sequencerSetNextTable = dlg.GetSequencerSetNextTable();
                vector<int64_type>::size_type sequencerSetNextTableCount = sequencerSetNextTable.size();
                for( vector<int64_type>::size_type i = 0; i < sequencerSetNextTableCount; i++ )
                {
                    sc.sequencerSetSelector.write( static_cast<int64_type>( i ) );
                    if( sc.sequencerSetLoad.isValid() )
                    {
                        sc.sequencerSetLoad.call();
                    }
                    /// \todo update more features later in this section
                    sc.sequencerSetNext.write( sequencerSetNextTable[i] );
                    if( sc.sequencerSetLoad.isValid() && sc.sequencerSetSave.isValid() )
                    {
            ...
            sc.sequencerMode.writeS( sequencerMode );
        }
        catch( const ImpactAcquireException& e )
        {
