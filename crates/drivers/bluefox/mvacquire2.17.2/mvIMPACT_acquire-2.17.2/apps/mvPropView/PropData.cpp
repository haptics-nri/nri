...
//-----------------------------------------------------------------------------
void PropertyObject::GetTransformedDict(
    wxPGChoices& soc,
    /// \brief Unfortunately since the property grid has been integrated into wxWidgets this
    /// parameter can no longer be passed to the grid property. However as the value '0' for
    /// features specifying the 'cfAllowValueCombinations' flag has to be treated different from
    /// the other values we keep the original code here.
    wxString* pEmptyString /* = 0 */ ) const
//-----------------------------------------------------------------------------
{
    const TComponentType type = GetComponent().type();
    if( type == ctPropInt )
...
//------------------------------------------------------------------------------
{
    ...
    if( !m_GridItemId )
    {
        ...
        case _ctrlSpinner:
            ...
            m_GridItemId->SetEditor( wxPGCustomSpinCtrlEditor::Instance()->GetEditor() );
            break;
        case _ctrlEdit:
