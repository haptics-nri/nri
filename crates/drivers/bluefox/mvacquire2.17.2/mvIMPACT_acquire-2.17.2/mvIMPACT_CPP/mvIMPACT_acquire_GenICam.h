...
public:
    ...
    /// \brief A boolean property. If enabled, the color of the LED changes depending on the state of the camera. Otherwise the LED is Off.
    /**
...
public:
    ...
    /// \brief An integer property. Retuns a unique Identifier value that correspond to the selected Source.
    /**
     *  Retuns a unique Identifier value that correspond to the selected Source.
     */
...
public:
        ...
        mvGevInterfaceLinkSpeed()
    {
        ...
        if( ( result = DMR_FindList( INVALID_ID, "mvGenTLConsumer", dmltDeviceDriverLib, 0, &hList ) ) != DMR_NO_ERROR )
        {
            ...
            msg << "Couldn't find list 'mvGenTLConsumer'(type: dmltDeviceDriverLib)";
            ExceptionFactory::raiseException( MVIA_FUNCTION, __LINE__, result, INVALID_ID, msg.str() );
        }
