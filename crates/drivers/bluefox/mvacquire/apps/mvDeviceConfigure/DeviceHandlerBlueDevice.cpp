...
//-----------------------------------------------------------------------------
{
    ...
    if( IsFirmwareUpdateMeaningless( boSilentMode, VersionFromString( ConvertedString( pDev_->firmwareVersion.readS() ) ), fileEntries[0].version_, firmwareUpdateDefaultFolder_, latestVersion ) )
    {
        return urOperationCanceled;
    }
