...
void wxPGVListBoxComboPopup::OnDrawItem( wxDC& dc, const wxRect& rect, size_t n) const
{
    ...
    if ( !m_combo->OnDrawListItem(dc,rect,(int)n,0) )
        dc.DrawText( GetString(static_cast<int>(n)), rect.x + 2, rect.y );
}

wxCoord wxPGVListBoxComboPopup::OnMeasureItem(size_t n) const
{
    int itemHeight = m_combo->OnMeasureListItem(static_cast<int>(n));
    if ( itemHeight < 0 )
...
{
    return static_cast<wxODCCount>(m_strings.GetCount());
}
                        ...
                        const wxString& name)
{
    ...
    return Create(parent, id, value, pos, size, static_cast<int>(chs.GetCount()),
...
{
	// Item counts in GUI components were changed in 2.7.0 (became unsigned)
#if wxCHECK_VERSION(2,7,0)
    wxCHECK_RET( (n < GetCount()), _T("invalid index in wxPGOwnerDrawnComboBox::Delete") );
#else
...
wxString wxPGOwnerDrawnComboBox::GetString(wxODCIndex n) const
{
	// Item counts in GUI components were changed in 2.7.0 (became unsigned)
#if wxCHECK_VERSION(2,7,0)
    wxCHECK_MSG( (n < GetCount()), wxEmptyString, _T("invalid index in wxPGOwnerDrawnComboBox::GetString") );
#else
...
void wxPGOwnerDrawnComboBox::SetString(wxODCIndex n, const wxString& s)
{
	// Item counts in GUI components were changed in 2.7.0 (became unsigned)
#if wxCHECK_VERSION(2,7,0)
    wxCHECK_RET( (n < GetCount()), _T("invalid index in wxPGOwnerDrawnComboBox::SetString") );
#else
